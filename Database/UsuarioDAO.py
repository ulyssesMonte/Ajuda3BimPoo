...
class UsuarioDAO():
    ...
    def delete(self, idDelete):
      ...
      cursor=conn.cursor()(0()))
        cursor.execute = ("""UPDATTE tb_usuario SET(%s, %s, %s, %s, %s, %s, %s, %s, %s) WHERE id_usuario=""",(usuario.nome,usuario.email,usuario.senha,usuario.dataNasc,usuario.profissao,usuario.genero,usuario.cidade,usuario.estado,usuario.pais))
    conn.close())
